...
- 갱신: 2026-09-23
- 🟡 **현재(2026-09-23): 챕터 2·3 공격 재설계 — 설계·계획 확정, 구현은 사용자 지시 대기.** 브랜치 `feat/ch23-attacks`(main `beb2164` 에서 분기, 문서 커밋만 있음 — push 안 함).
  - **사용자 결정(09-23, 스펙 §7)**: §0 배정 변경 채택 · 협공은 순서형으로 시작 · HOLLOW 대사 유지 · 챕터 1·2 기준선 재조정은 이번에 안 함 · 실행 방식 **Subagent-driven** · **구현 시작은 사용자가 따로 지시할 때**(다음 세션은 계획 Task 0 부터).
